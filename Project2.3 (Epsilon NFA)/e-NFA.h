    ...
    while(getline(fin,cuvant) and !Check_End(cuvant) and !Check_other_section(cuvant)) {
        if (cuvant.find('#') != string::npos or cuvant.empty()) {
            ...
            if (ok1 or cuvant.empty())
                ...
                if (cuvant.find("ε") != string::npos)
                    cuvant = cuvant.substr(cuvant.find("ε"), cuvant.find(' '));
                else {
                    int li = 0;
                    while (!isalpha(cuvant[li]))
                        li++;
                    if(li + 1 < cuvant.size())
                        cuvant = cuvant.substr(li, cuvant.find(' '));
                }
                for (auto &x : alfab)
                    if (x.find(cuvant[0]) != string::npos and cuvant[0] != ' ')
                        ok = true;
                int count = 0;
                if (cuvant.find("ε") != string::npos)
                    count = 1;
